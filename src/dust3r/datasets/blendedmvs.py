...
class BlendedMVSContinuous_Multi(BlendedMVS_Multi):
    """Contiguous windows from BlendedMVS for frontend training."""

    def __init__(self, *args, ROOT, split=None, contiguous_step=1, **kwargs):
        self.contiguous_step = contiguous_step
        super().__init__(*args, ROOT=ROOT, split=split, **kwargs)

    def _load_data(self):
        super()._load_data()
        self.all_sequences = []
        required = 1 + self.contiguous_step * max(self.num_views - 1, 0)
        for scene_dir, scene_data in self.data_dict.items():
            num_frames = len(scene_data["basenames"])
            if num_frames < required:
                continue
            max_start = num_frames - required
            for start_idx in range(max_start + 1):
                self.all_sequences.append((scene_dir, start_idx))
    ...
    def get_stats(self):
        return f"{len(self)} contiguous sequences from {self.num_scenes} scenes"

    def _get_views(self, idx, resolution, rng: np.random.Generator, num_views):
        scene_info, start_idx = self.all_sequences[idx]
        basenames = self.data_dict[scene_info]["basenames"]
        imgs_idxs = [start_idx + self.contiguous_step * offset for offset in range(num_views)]
        views = []
        ordered_video = True
        for view_idx in imgs_idxs:
            scene_dir = osp.join(self.ROOT, scene_info)
