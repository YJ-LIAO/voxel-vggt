...
class BaseMultiViewDataset(EasyDataset):
    ...
    def __init__(
        ...
        *,  # only keyword arguments
        ...
        self.max_bad_view_retries = max(int(max_bad_view_retries), 0)
    ...
    def __getitem__(self, idx):
        ...
        resolved_idx = idx
        retry_count = 0
        while retry_count <= self.max_bad_view_retries:
            try:
                views = self._get_views(resolved_idx, resolution, self._rng, nview)
                return self._finalize_views(
                    views=views,
                ...
                if not self._is_retryable_bad_view_error(exc):
                    raise
                retry_count += 1
                if retry_count > self.max_bad_view_retries:
                    raise RuntimeError(
                        "Exceeded max_bad_view_retries while skipping bad samples. "
                        f"last_idx={resolved_idx}, retries={self.max_bad_view_retries}, error={exc}"
                    ) from exc
                LOGGER.warning(
                    "Skipping bad sample idx=%s (retry %d/%d): %s",
                    resolved_idx,
                    retry_count,
                    self.max_bad_view_retries,
                    exc,
                )
                resolved_idx = self._sample_retry_index(resolved_idx)
        raise RuntimeError("Failed to load a valid multiview sample after retries.")

    def _finalize_views(self, views, resolved_idx: int, ar_idx: int, nview: int):
        ...
        for v, view in enumerate(views):
            ...
            view["idx"] = (resolved_idx, ar_idx, v)
    ...
    @staticmethod
    def _is_retryable_bad_view_error(exc: AssertionError) -> bool:
        message = str(exc)
        retryable_patterns = (
            "Bad principal point in view=",
            "NaN in camera pose for view",
            "NaN in depthmap for view",
        )
        return any(pattern in message for pattern in retryable_patterns)

    def _sample_retry_index(self, current_idx: int) -> int:
        dataset_len = len(self)
        if dataset_len <= 1:
            return current_idx
        retry_idx = int(self._rng.integers(0, dataset_len))
        if retry_idx == current_idx:
            retry_idx = (retry_idx + 1) % dataset_len
        return retry_idx
