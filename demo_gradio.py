# OVGGT Gradio Demo
# Ported from StreamVGGT (https://huggingface.co/lch01/StreamVGGT)
...
print("Initializing and loading OVGGT model...")
...
def run_model(target_dir, model) -> dict:
    """
    Run the OVGGT model on images in the 'target_dir/images' folder and return predictions.
    """
...
def gradio_demo(
    ...
    if not os.path.isdir(target_dir) or target_dir == "None":
        return "", "No valid target directory found. Please upload first.", gr.update(choices=["All"], value="All", interactive=True)
    ...
    log_msg = f"Reconstruction Success ({len(all_files)} frames). Drag to rotate, scroll to zoom."

    viewer_html = build_model_viewer_html(glbfile)
    return viewer_html, log_msg, gr.update(choices=frame_filter_choices, value=frame_filter, interactive=True)


# -------------------------------------------------------------------------
# 4b) Build model-viewer HTML
# -------------------------------------------------------------------------
def build_model_viewer_html(glbfile: str) -> str:
    """
    Return an HTML snippet that uses Google's <model-viewer> (WebGL-based)
    to render the given GLB file path served by Gradio's static file server.
    """
    if not glbfile or not os.path.exists(glbfile):
        return "<p style='color:red;'>GLB file not found.</p>"
    # Gradio 6.x serves files at /gradio_api/file=<abs-path>
    glb_url = f"/gradio_api/file={glbfile}"
    return f"""
<style>
  model-viewer {{
    width: 100%;
...
<div class="mv-wrap">
  ...
  <a href="{glb_url}" download="reconstruction.glb">
    <button class="mv-download">⬇ Download GLB</button>
  </a>
</div>
"""
...
def update_visualization(
    ...
    if is_example == "True":
        return "", "No reconstruction available. Please click the Reconstruct button first."
    ...
    if not os.path.exists(predictions_path):
        return "", f"No reconstruction available at {predictions_path}. Please run 'Reconstruct' first."
    ...
    viewer_html = build_model_viewer_html(glbfile)
    return viewer_html, "Updating Visualization"
...
MODEL_VIEWER_HEAD = '<script type="module" src="https://ajax.googleapis.com/ajax/libs/model-viewer/3.5.0/model-viewer.min.js"></script>'

with gr.Blocks(head=MODEL_VIEWER_HEAD) as demo:
    ...
    gr.HTML(
        """
    <h1>OVGGT: O(1) Constant-Cost Streaming Visual Geometry Transformer</h1>
    <p>
    <a href="https://huggingface.co/lch01/StreamVGGT">Base Model (Hugging Face)</a>
    </p>
    
    <div style="font-size: 16px; line-height: 1.5;">
    <p>Upload a video or a set of images to create a 3D reconstruction of a scene or object. OVGGT takes these images and generates a 3D point cloud, along with estimated camera poses, in a training-free streaming manner.</p>
    ...
    <p><strong style="color: #0ea5e9;">Please note:</strong> <span style="color: #0ea5e9; font-weight: bold;">OVGGT typically reconstructs a scene in less than 1 second. However, visualizing 3D points may take tens of seconds due to third-party rendering, which are independent of OVGGT's processing time. </span></p>
    </div>
    """
    )
    ...
    with gr.Row():
        ...
        with gr.Column(scale=4):
            with gr.Column():
                ...
                reconstruction_output = gr.HTML(value="", label="3D Viewer")

            with gr.Row():
                submit_btn = gr.Button("Reconstruct", scale=1, variant="primary")
                clear_btn = gr.ClearButton(
                    [input_video, input_images, log_output, target_dir_output, image_gallery],
                    scale=1,
                )
                clear_btn.click(fn=clear_fields, inputs=[], outputs=[reconstruction_output])
    ...
    def example_pipeline(
        ...
        frame_filter = "All"
        viewer_html, log_msg, dropdown = gradio_demo(
            target_dir, conf_thres, frame_filter, mask_black_bg, mask_white_bg, show_cam, mask_sky, prediction_mode
        )
        return viewer_html, log_msg, target_dir, dropdown, image_paths
    ...
    demo.queue(max_size=20).launch(show_error=True, share=True, theme=theme, css=custom_css, allowed_paths=[os.getcwd()])